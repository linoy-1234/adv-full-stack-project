...
export function LoginPage({ onLogin, onGoToRegister, onBackToHome }: LoginPageProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10" style={{ backgroundColor: "#FAF8F5", position: "relative" }}>
      ...
      <div className="flex flex-col items-center gap-2 mb-8 relative z-10">
        ...
        <p className="text-sm text-center" style={{ color: "#6B7280" }}>Cancer Treatment Coordination Portal</p>
      </div>

      <div className="w-full max-w-md rounded-3xl shadow-lg p-8 bg-white relative z-10">
        <h2 className="text-base font-semibold text-center mb-6" style={{ color: "#2C3E2D" }}>Sign In</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {error && (
            <p className="text-sm text-center rounded-xl px-4 py-2.5" style={{ backgroundColor: "#FEF2F2", color: "#DC2626" }}>{error}</p>
          )}

          <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl text-white text-sm hover:opacity-90 disabled:opacity-60 mt-1" style={{ backgroundColor: "#7CAE8E" }}>
        ...
        <div className="mt-5 p-4 rounded-2xl text-xs space-y-1.5" style={{ backgroundColor: "#F0FAF4", border: "1.5px solid #A7F3D0" }}>
          <p className="font-semibold text-[#2D4739] mb-2">Demo Credentials</p>
          <p style={{ color: "#4B7A5E" }}>🌸 Patient: <strong>sarah.cohen@email.com</strong> / <strong>patient1</strong></p>
          <p style={{ color: "#4B7A5E" }}>🌿 Also linked: <strong>rachel.m@email.com</strong> or <strong>tamar.katz@email.com</strong></p>
          <p style={{ color: "#2D4739" }}>🩺 Oncologist: <strong>dr.goldstein@oncolog.com</strong> / <strong>onco123</strong></p>
          <p style={{ color: "#2D4739" }}>🔬 Lab Staff: <strong>noa.lab@oncolog.com</strong> / <strong>lab123</strong></p>
        </div>
      </div>

      <button onClick={onGoToRegister} className="mt-6 text-sm underline underline-offset-2 hover:opacity-70 relative z-10" style={{ color: "#7CAE8E" }}>
